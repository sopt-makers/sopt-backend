...
reload_nginx() {
    ...
    echo "set \$actuator_url http://127.0.0.1:${ACTUATOR_PORT};" | sudo tee /etc/nginx/conf.d/actuator-url.inc
    
    sudo nginx -s reload
    echo "Current running Port after switching: $(sudo cat /etc/nginx/conf.d/app-url.inc)"
    echo "Current running Actuator Port after switching: $(sudo cat /etc/nginx/conf.d/actuator-url.inc)"
}
package org.sopt.app.common.jwt.service;
...
import org.sopt.app.common.external.auth.AuthClient;
import org.sopt.app.common.exception.ClientException;
import org.sopt.app.common.exception.JwkException;
...
public class JwkProvider {
    ...
    private PublicKey resolvePublicKey(String kid) {
        try {
            ...
            throw new JwkException(ErrorCode.JWK_FETCH_FAILED);
        }
    }
    ...
    private JWK findJwkByKeyId(JWKSet jwkSet, String kid) {
        return jwkSet.getKeys().stream()
                ...
                .findFirst()
                .orElseThrow(() -> new JwkException(ErrorCode.JWK_KID_NOT_FOUND));
    }

    private RSAPublicKey convertToPublicKey(JWK jwk) {
        if (!(jwk instanceof RSAKey rsaKey)) {
            throw new JwkException(ErrorCode.JWK_INVALID_FORMAT);
        }
